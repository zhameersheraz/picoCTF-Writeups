...
The hint mentioned the flag format is `PICOCTF{}` — this suggested a simple **A1Z26 cipher** where each number corresponds to its position in the alphabet:

```
A=1, B=2, C=3, D=4 ... Z=26
```

I used the online tool **dcode.fr** to decode it:
...
I added the `{` and `}` brackets as shown in the image:
...
```
numbers.png → Read numbers → A1Z26 decode → Add brackets → Flag
```
...
Simply use the alphabet:
```
A=1, B=2, C=3 ... and count each number to its letter
```
